...
struct ImFont
{
	...
	ImU8 Used8kPagesMap[ ( IM_UNICODE_CODEPOINT_MAX + 1 ) / 8192 /
						 8 ]; // 1 bytes if ImWchar=ImWchar16, 16 bytes if ImWchar==ImWchar32. Store 1-bit for each
	...
	{
		return CalcWordWrapPosition( FontSize * scale, text, text_end, wrap_width );
	}
#endif
	...
	IMGUI_API void AddRemapChar(
		ImWchar from_codepoint, ImWchar to_codepoint,
		bool overwrite_dst ); // , bool overwrite_dst = true); // Makes 'from_codepoint' character points to
							  // 'to_codepoint' character. Currently needs to be called AFTER fonts have been built.
