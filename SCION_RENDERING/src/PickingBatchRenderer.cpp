...
{
	auto newSprite = std::make_shared<PickingGlyph>( PickingGlyph{
		.topLeft = PickingVertex{ .position = model * glm::vec4{ spriteRect.x, spriteRect.y + spriteRect.w, 0.f, 1.f },
								  .uvs = glm::vec2{ uvRect.x, uvRect.y + uvRect.w },
								  .color = color,
								  .uid = id },
		.bottomLeft = PickingVertex{ .position = model * glm::vec4{ spriteRect.x, spriteRect.y, 0.f, 1.f },
									 .uvs = glm::vec2{ uvRect.x, uvRect.y },
									 .color = color,
									 .uid = id },
		.topRight =
			PickingVertex{ .position =
							   model * glm::vec4{ spriteRect.x + spriteRect.z, spriteRect.y + spriteRect.w, 0.f, 1.f },
						   .uvs = glm::vec2{ uvRect.x + uvRect.z, uvRect.y + uvRect.w },
						   .color = color,
						   .uid = id },
		.bottomRight =
			PickingVertex{ .position = model * glm::vec4{ spriteRect.x + spriteRect.z, spriteRect.y, 0.f, 1.f },
